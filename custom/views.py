...
from .models import AdministrativePost, Village, SubVillage
...
def load_subvillages(request):
	aldeia_id = request.GET.get('aldeia')
	subVillage = SubVillage.objects.filter(village=aldeia_id).order_by('name')
	return render(request, 'custom/subVillage_dropdown.html', {'subVillage': subVillage})
